...
const char * const BITCOIN_CONF_FILENAME = "viltracoin.conf";
const char * const BITCOIN_PID_FILENAME = "viltracoin.pid";
...
#else
    const char* pszModule = "viltracoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Viltracoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Viltracoin";
#else
    // Unix
    return pathRet / ".viltracoin";
#endif
#endif
}
